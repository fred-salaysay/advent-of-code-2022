...
const guide = []
    .concat(lower.map((x, index) => [x, index + 1]))
    .concat(upper.map((x, index) => [x, index + 1 + 26]));
...
const duplicate = input
    ...
    .map((sack) =>
        [...new Set(sack[0])].find((y) =>
            [...new Set(sack[1])].some((z) => y === z),
        ),
    )
    .map((dup) => guide.find((x) => (x[0] === dup ? x[1] : false)))
